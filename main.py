...
from hmk1.util import extract_public_key, verify_artifact_signature
from hmk1.merkle_proof import (
    DefaultHasher,
    verify_consistency,
    verify_inclusion,
    compute_leaf_hash,
...
def get_log_entry(log_index, debug=False):
    ...
    """
    try:
        raw_entry = requests.get(f"{LOG_ENTRY_URL}{log_index}", timeout=10).json()
        if debug:
...
def inlclusion_arg_sane(log_index_arg, artifact_arg):
    ...
    """
    return (
        isinstance(log_index_arg, int)
        and log_index_arg > 0
        and isinstance(artifact_arg, str)
        and len(artifact_arg) > 0
    )
...
def get_consistency_data(current_tree_size):
    ...
    """
    checkpoint_response_url = "https://rekor.sigstore.dev/api/v1/log?stable=true"
    ...
    try:
        ...
        print("===================================")
        print("\tcurrent tree size:")
        print(current_tree_size)
        print("===================================")
        print("\tproof_response_data here:")
        print(proof_response_data)
        print("===================================")
        proof_hashes = proof_response_data["hashes"]

        return (proof_hashes, checkpoint_tree_size, checkpoint_root_hash)
...
def main():
    ...
    if args.inclusion:
        ...
        if not inlclusion_arg_sane(args.inclusion, args.artifact) or debug_flag:
            print(
                f"bad arguments, I will use default arguments: {132216490}, file:{'artifact.md'}"
            )
        ...
        (
            ...
            hashes,
            root_hash,
        ) = signature_inclusion_proof(entry)

        verify_artifact_signature(signature, public_key, args.artifact)
        verify_inclusion(
            DefaultHasher, proof_log_index, tree_size, leaf_hash, hashes, root_hash
        )
    ...
    if args.consistency:
        ...
        if (
            not consistency_arg_sane(args.tree_id, args.tree_size, args.root_hash)
            or debug_flag
        ):
            ...
            args.tree_size, args.root_hash = (
                proof_data[3],
                proof_data[6],
            )  # we only need tree size and root hash

        # params: hasher, size1, size2, proof, root1, root2
        # note size2 >= size1 always, implied size 1 latest checkpoint data
        (proof_hashes, checkpoint_tree_size, checkpoint_root_hash) = (
            get_consistency_data(args.tree_size)
        )
        verify_consistency(
            DefaultHasher,
